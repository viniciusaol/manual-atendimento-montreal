...
def fetch_and_calculate_sla():
    headers = {'api_access_token': '225uXmdyRZfcQ4HhGeYzMZwF'}
    base = 'https://chat.vxautomation.com.br/api/v1/accounts/1'

    all_conversations = []
    page = 1
    max_pages = 10 # 250 conversas mais recentes para resposta super rápida

    print("Buscando conversas no Chatwoot...", flush=True)
    ...
    while page <= max_pages:
        try:
            ...
            if r.status_code != 200:
                print(f"Página {page} retornou status {r.status_code}", flush=True)
                break
            payload = r.json().get('data', {}).get('payload', [])
            if not payload:
                print(f"Página {page} vazia. Encerrando coleta.", flush=True)
                break
            all_conversations.extend(payload)
            print(f"Página {page}: +{len(payload)} conversas (Total: {len(all_conversations)})", flush=True)
            page += 1
        except Exception as e:
            print(f"Erro na página {page}: {e}", flush=True)
            break

    print(f"Total de conversas carregadas: {len(all_conversations)}", flush=True)
    ...
    report = {
        ...
        "periods": {
            "mes_atual": process_slice(convs_mes_atual if convs_mes_atual else all_conversations[:50], "Mês Atual (Setembro/2026)"),
            "mes_anterior": process_slice(convs_mes_anterior if convs_mes_anterior else all_conversations[50:150], "Mês Anterior (Agosto/2026)"),
            "ultimos_30d": process_slice(convs_30d if convs_30d else all_conversations, "Últimos 30 Dias"),
            "ultimos_7d": process_slice(convs_7d if convs_7d else all_conversations[:30], "Últimos 7 Dias")
        }
    }

    out_file = "data/chatwoot_sla_report.json"
    with open(out_file, "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)

    print(f"Relatório gerado com sucesso em: {out_file}", flush=True)

if __name__ == "__main__":
    fetch_and_calculate_sla()
